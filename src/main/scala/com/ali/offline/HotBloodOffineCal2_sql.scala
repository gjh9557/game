...
object HotBloodOffineCal2_sql {
  ...
  def getTimeMills(castdata: String) = {
    //  val baseDatestr = castdata.concat(" 00:00:00")
    val sdf = new SimpleDateFormat(CommonUtils.loadApolicationpropertie().getString(CommonData.TIME_PATTERN2))
    val date = sdf.parse(castdata)
    val baseDateMillions = date.getTime
    baseDateMillions
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val filter = ReadEsData(sc).toDF("userName", "eventType", "time", "ip")
    filter.createOrReplaceTempView("tb_tmp")
    spark.sqlContext.cacheTable("tb_tmp")
    ...
    //注册udf自定义函数
    spark.udf.register("getTime", (time: String) => getTimeMills(time))
    //注册时间大于今天小于明天

    val newUser = spark.sql(
      s"""
select
count(distinct(userName)) `新增用户`
from tb_tmp
where eventType=1 and (getTime(time)>=UNIX_TIMESTAMP('$baseData','yyyy-MM-dd') * 1000 and
getTime(time)<UNIX_TIMESTAMP(DATE_ADD('$baseData',1),'yyyy-MM-dd') * 1000)
  """.stripMargin).rdd.first().getAs[Long]("新增用户")
    ...
    /**
      ...
      *
      */
    val activeuser = spark.sql(
      s"""
select
count(distinct(userName)) `活跃用户`
from tb_tmp
where (eventType in ('1','2')) and (getTime(time)>=UNIX_TIMESTAMP('$baseData','yyyy-MM-dd') * 1000 and
getTime(time)<UNIX_TIMESTAMP(DATE_ADD('$baseData',1),'yyyy-MM-dd') * 1000)
  """.stripMargin).rdd.first().getAs[Long]("活跃用户")
    activeuser
